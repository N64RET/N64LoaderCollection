...
public enum Zelda64Version {
    ...
    //Animal Crossing
    Ac("forest@srd038J.srd.nintendo.co.jp", "01-02-14 16:37:16");

    public String mTeam;
    public String mDate;

    private Zelda64Version(String team, String date) {
        mTeam = team;
        mDate = date;
    }

    public String GetBuildName() {
        if (mTeam != null && mDate != null)
            return mTeam + " " + mDate;
        return "Invalid";
    }

    public static Zelda64Version FromString(String team, String date) {

    	var values = Zelda64Version.values();
    	for (int i = 0; i < values.length; i++)
    	{
    		if (team.equals(values[i].mTeam) && date.equals(values[i].mDate))
    			return values[i];
    	}
